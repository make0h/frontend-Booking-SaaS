...
export default function TeachersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(error.response?.data || `Error al ${modalMode === 'create' ? 'registrar' : 'actualizar'} al instructor`);
    }
  };

  const handleDeleteTeacher = async (id: number, teacherName: string) => {
    if (confirm(`¿Estás seguro de dar de baja al instructor "${teacherName}"?`)) {
      try {
        await api.delete(`/users/employees/${id}`);
        fetchTeachers();
      } catch (error: any) {
        alert(error.response?.data || 'Error al eliminar el instructor.');
      }
    }
  };
            ...
            <div key={teacher.id} className="bg-slate-900 rounded-2xl p-6 border border-slate-800 shadow-xl hover:border-cyan-500/50 transition-all flex flex-col justify-between group">
              ...
              <div className="flex justify-between items-start mb-4">
                ...
                */}
                ...
                <button 
                    onClick={() => openEditModal(teacher)} // Usa 'teacher' en el archivo de profesores
                ...
                <button 
                    onClick={() => handleDeleteTeacher(teacher.id, teacher.name)} // Usa handleDeleteTeacher en el otro
